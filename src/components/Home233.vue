    ...
    <div v-if="pdfSrc" class="w-full flex h-full">
      ...
      >
        <div
          ...
          :key="index"
          class="cell-item mb-4 p-4 bg-gray-100 rounded-lg shadow-md"
        >
          <p class="text-gray-800 font-semibold">发票 {{ index + 1 }}</p>
          <p class="text-gray-600 text-sm">金额: ￥{{ item.amount }}</p>
          <button
            class="mt-2 px-4 py-2 bg-red-500 text-white rounded cursor-pointer hover:bg-red-600"
