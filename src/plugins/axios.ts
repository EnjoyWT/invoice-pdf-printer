...
export const GET = async <T = any>(
  url: string,
  params?: Record<string, any> & { onDownloadProgress?: AxiosRequestConfig["onDownloadProgress"] }
): Promise<T> => {
  const { onDownloadProgress, ...rest } = params || {}
  const res: AxiosResponse<T> = await service.get<T>(url, { params: rest, onDownloadProgress })
  return res.data
}
...
export const POST = async <T = any>(
  url: string,
  body?: any & { onDownloadProgress?: AxiosRequestConfig["onDownloadProgress"] }
): Promise<T> => {
  const { onDownloadProgress, ...rest } = (body || {}) as Record<string, any>
  const res: AxiosResponse<T> = await service.post<T>(url, rest, { onDownloadProgress })
  return res.data
}
